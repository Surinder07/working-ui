...
const Dashboard = (props) => {
    ...
    useEffect(() => {
        if (props.user.role) {
            dashboardService.getData(props.user.role).then((res) => {
                try {
                    ...
                    <DashboardCard className={DashboardStyles.graph}>
                        {lineGraphData ? (
                            <div style={{position: "relative", height: "100%", minHeight: "350px", display: "flex", justifyContent: "center", alignItems: "center"}}>
                                <Line data={lineGraphData} options={areaConfig(getLineGraphTitle().title, getLineGraphTitle().description, getLineGraphTitle().x, getLineGraphTitle().y)} />
                                {lineGraphData.noData && <p style={{position: "absolute", top: "50%"}}>No Data to show</p>}
                            </div>
                        ) : (
                            <p>Loading...</p>
                        )}
                        {pieGraphData ? (
                            pieGraphData === "noData" ? (
                                ...
                                <div style={{position: "relative", minHeight: "350px", display: "flex", justifyContent: "center", alignItems: "center"}}>
                                    <Pie data={pieGraphData} options={pieConfig("Employee Trends", "Current Year ( 2022-2023 )", "Month", "Invoice Amount")} />
                                    {pieGraphData.noData && <p style={{position: "absolute", top: "50%"}}>No Data to show</p>}
                                </div>
                            )
                        ) : (
                            <p>Loading...</p>
