    ...
    return (
        <nav className={joinClasses(NavbarStyles.nav, PageStyles.pagePadding)}>
            ...
            <div className={joinClasses(NavbarStyles.navEl, NavbarStyles.menu, openMenu ? NavbarStyles.openMenu : NavbarStyles.closeMenu)}>
                <LinkedImage className={joinClasses(NavbarStyles.logo, NavbarStyles.showMobile)} link={logo.white.link} src={logo.white.src} alt={logo.white.alt} heightOrient keepQuality />
                <ul className={NavbarStyles.menuItems}>
                    {NavLinks.map((nav, i) =>
                        nav.link ? (
                            ...
                            <li key={`menu${i}`} className={joinClasses(NavbarStyles.menuItem, NavbarStyles.expandableMenu, props.pageInfo.activeMenu === nav.activeKey && NavbarStyles.activeMenu)}>
                                <p>{nav.title}</p>
                                {
                                    <ul className={NavbarStyles.subMenu}>
                                        {nav.dropdown.map((drop, i) =>
                                            getMenuLink(`sub-menu${i}`, drop.title, drop.link, props.pageInfo.activeSubMenu === drop.activeKey && NavbarStyles.menuItemActive)
                                        )}
                                    </ul>
                                }
                            </li>
