...
const Table = (props, ref) => {
    ...
    useEffect(() => {
        if (props.data.length > 0) {
            let keyList = Object.keys(props.data[0]).filter((item) => item !== "subData" && item !== "internalId");
    ...
    return (
        <div className={TableStyles.table} style={{ gridTemplateColumns: `repeat(${colNum}, auto)` }} ref={ref}>
            {/* headers */}
            {props.data[0] && props.data[0].subData && <div className={TableStyles.headerCell}></div>}{/* An empty header for expand button in the body below */}
