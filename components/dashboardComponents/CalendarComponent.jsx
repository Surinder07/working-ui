...
const CalendarComponent = () => {
    ...
    useEffect(() => {
        let newDaysObj = referenceDays.map((day) => {
            ...
            workingDays.filter((workingDay) => isSameDay(parseISO(workingDay.endDatetime), day.date))
                .map(workingDay => workedDaysList.push({
                    type: 'Out',
                    time: workingDay.endDatetime.split('T')[1]
                }))
            if (workedDaysList.length > 0) {
                return { ...day, worked: workedDaysList }
            }
            return day;
        })
        setDays(newDaysObj);
                ...
                <div className={CalendarStyles.calenderContainer}>
                    <div className={CalendarStyles.innerCalenderContainer}>
                        <div className={CalendarStyles.dateNav}>
                            ...
                            <ArrowRight className={`${CalendarStyles.arrowIcon} ${disableNext && CalendarStyles.disabledIcon}`}
                                onClick={nextMonth} />
                        </div>
                        <div className={CalendarStyles.daysContainer}>
                            {
                                DaysOfWeekShort.map((day, i) => (
                                    <div className={CalendarStyles.days} key={`dayName_${i}`}>{day}</div>
                                ))
                            }
                        </div>
                            ...
                            {days.map((day, i) => (
                                <div
                                    ...
                                    className={classNames(
                                        ...
                                        holidays.filter(holiday => holiday.type === 'public').some((holiday) => isSameDay(parseISO(holiday.startDatetime), day.date)) && CalendarStyles.publicHoliday,
                                        holidays.filter(holiday => holiday.type === 'org').some((holiday) => isSameDay(parseISO(holiday.startDatetime), day.date)) && CalendarStyles.organizationHoliday
                                    )}
                                >
