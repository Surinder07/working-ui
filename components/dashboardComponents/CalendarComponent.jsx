...
const CalendarComponent = () => {
  ...
  useEffect(() => {
    ...
    newDaysObj = newDaysObj.map((day) => {
      let publicHoliday, organizationHoliday;
      holidays.filter((holiday) => holiday.type === "public").some((holiday) => (isSameDay(parseISO(holiday.startDatetime), day.date) ? (publicHoliday = true) : (publicHoliday = false)));
      holidays.filter((holiday) => holiday.type === "org").some((holiday) => (isSameDay(parseISO(holiday.startDatetime), day.date) ? (organizationHoliday = true) : (organizationHoliday = false)));
      return { ...day, publicHoliday: publicHoliday, organizationHoliday: organizationHoliday };
    });
    setDays(newDaysObj);
  }, [referenceDays]);
